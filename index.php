...
<main>
    ...
    <div class="content">
        ...
        <div class="content__info">
            <p>
                <?php
                $text = "Привет, я инженер по автоматизации. Практически каждый день на работе я подключаю разные 
                устройства друг к другу и пишу для них программы на различных языках. Также я разрабатываю программное 
                обеспечение для ПК, проектирую БД, добавляю новый функционал для корпоративного портала и пишу ботов 
                для телеграм. В общем делаю все и сразу, но сейчас хочу сосредоточиться на чем-то одном - стать 
                профессионалом в области веб-разработки. Люблю горные велосипеды и горные горы.";
                echo "<span class='color_first'>" . substr_replace($text, '</span>', strpos($text, '.') + 1, 0);
                ?>
            </p>
        </div>
        <div class="content__review">
            <p>
                <?php
                $text = "Обучение мне нравится, но, возможно, следовало бы разделить студентов по уровням знания, чтобы 
                все обучались своим темпом.";
                $arr = explode(" ", $text);
                $text = "";
                foreach ($arr as $key => $value){
                    if (($key + 1) % 2 == 0){
                        $text .= "<span class='color_first'>$value </span>";
                    }
                    else{
                        $text .= "<span class='color_second'>$value </span>";
                    }
                }
                echo $text;
                ?>
